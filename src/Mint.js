...
const Mint = () => {
  ...
  const handleNetworkSwitch = async (networkName) => {
    ...
    alert(`Switched to ${networkName} successfully !`)
  }
  ...
  useEffect(async () => {
    ...
    switch (window.ethereum.networkVersion) {
      case "1":
        activeNetwork = 'Ethereum Mainnet';
        break;
    ...
    setBalanceInfo({
      ...
      // balance: balance  // using this Error: Objects are not valid as a React child (found: object with keys {_hex, _isBigNumber}). If you meant to render a collection of children, use an array instead.
    });
  ...
  return (
    <div className='Mint'>
      ...
      <p>You are currently connected to {network} network !</p>
      ...
      <div className='spn'>
        ...
        <span>

          {
            items.map((item, i) => (
              <div key={i}>
                <span className="disp">TokenId: {item.tokenIndex} | </span>
                <span className="disp"> Token: {item.token._hex}</span>
              </div>
            ))
          }
